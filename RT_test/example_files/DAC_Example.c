...
//*****************************************************************************																				
printf("\nDIO Functions\n");													  										
out8(DIO_CTLREG,0x10);		// Port A : Output B; Input Port C : out|out 									
					
dio_in=in8(DIO_PORTA); printf("Port A : %02x\n",dio_in);	// Read back																
out8(DIO_PORTB,0xf);						// Write to Port B  		
...
for(i=0x08fff;i<0xffff;i=i+0x40) {
	out16(DA_CTLREG,0x0a23);			// DA Enable, #0, #1, SW 5V unipolar		2/6
  	...
  	out16(DA_FIFOCLR, 0);			// Clear DA FIFO  buffer
	out16(DA_Data,(short) i);																							printf("DAC Data [%4x]: %4x\r", i, i);	// print DAC													
  	fflush( stdout ); 
  	delay(5);
  	}
